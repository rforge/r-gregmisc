...
ooplot.default <- function(data, width=1, space=NULL, names.arg=NULL,
                           legend.text=NULL, beside=TRUE, horiz=FALSE,
                           density=NULL, angle=45,
                           type=c("xyplot","linear","barplot"), 
                           ...
                           cex.names=par("cex.axis"), cex.values=0.7,
                           ...
                           grid.col="black", add=FALSE, by.row=FALSE,
                           ...) 
{
  ...
  heightscale <- ""
  heightsymbol <- ""
  if (max(height, na.rm=TRUE)>1E03) {
    height <- height/1E03
    heightscale <- "Kilo"
    heightsymbol <- "k"
  }
  if (max(height, na.rm=TRUE)>1E06) {
    height <- height/1E06
    heightscale <- "Mega"
    heightsymbol <- "M"
  } 
  ...
  if ((logx && horiz) || (logy && !horiz))
    {
      ...
      if (plot.ci)
        rectbase <- rangeadj <- (1.0 * min(c(height, ci.l)))
      else
        rectbase <- rangeadj <- (1.0 * min(height))
  ...
  ##xlim <- optlim(xlim, logx)
  ##ylim <- optlim(ylim, logy)
  if (FALSE) {
  ...
  if(plot) ##-------- Plotting :
    {
      ...
      if (type=="xyplot" || type=="linear") {
        ...
        for (i in 1:NR) {
          ...
          ##values=paste(format(y, digits=4), "  [±", format(dy, digits=4), "]", sep="")
          ##errbar(mp, y, yplus, ymin, log=log, add=TRUE)
          chh <- par()$cxy[2]
          chw <- par()$cxy[1]
          if (logy) {
            factor=(yrange+chh)/yrange
            ##print(factor)
            text(xrange, 1.1*yrange, labels=values, adj=c(0, 1), srt=0, 
                 cex=cex.values, bg="white", col="navy")
          } else {
            text(xrange, yrange+chh/2, labels=values, adj=c(0, 1), srt=0, 
                 cex=cex.values, bg="white", col="navy")
          }
        }
      }
      ...
      if (type=="barplot") {
        ...
        if (beside) {
          ...
          text(w.l, c(height)+chh/2, labels=values, adj=c(0, 1), srt=0, 
               cex=cex.values, bg="white", col="navy")
        } else {
            for (i in 1:NC) {
              ...
              values=format(c(height[, i]), digits=4)
              text(w.l, c(height[, i])+chh/2, labels=values, adj=c(0, 1), srt=0, 
                   cex=cex.values, bg="white", col="navy")
            }
          }
      }
      ...
      if (axes)
        {
          ...
          if (type=="barplot") {
            ...
            if(plot.grid) {
              ...
              if (type=="linear") {
                axis(if(horiz) 2 else 1, pos=ylim[1], at=at.l,
                     labels = names.arg, cex.axis = cex.names, ...)
                ##axis(if(horiz) 2 else 1, pos=0, cex.axis=cex.axis, ...)
              } else {  
                axis(if(horiz) 2 else 1, pos=ylim[1], cex.axis=cex.axis, ...)
              }
            }
          }
        }
