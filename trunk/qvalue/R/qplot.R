qplot <- function(qobj,  rng=0.1,  ...) {
  ...
  plot(p2[q2<=rng], q2[q2<=rng], type="l", xlab="p-value",
       ylab="q-value", ...)
  
  plot(q2[q2<=rng], 1:sum(q2<=rng), type="l", xlab="q-value cut-off",
       ylab="significant tests", ...)

  
  plot(1:sum(q2<=rng), q2[q2<=rng]*(1:sum(q2<=rng)), type="l",
       xlab="significant tests", ylab="expected false positives", ...)
