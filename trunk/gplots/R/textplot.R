                             ...
                             cex, ... )
{
  ...
  if(missing(cex))
    {
      ...
      for(i in 1:20)
        {
          oldcex <- cex
          width <- strwidth(results,cex=cex)
          height <- strheight(results,cex=cex)
  ...
  # make the plot
  text( x=xpos, y=ypos, results, adj=c(0,1), cex=cex, ... )
...
textplot.matrix <- function(object,
                            halign=c("center","left","right"),
                            valign=c("center","top","bottom"),
                            cex, col.margin=2, row.margin=0.5,
                            ...
                            ... )
{
      ...
      for(i in 1:20)  # try to find the right text size
        {
          ...
          width  <- sum(
                        apply( object, 2,
                          ...
                          strwidth('M', cex=cex) * col.margin * ncol(object)
  ...
  rowheight<-strheight("M",cex=cex) * (1 + row.margin)
  colwidth<- apply( object, 2, function(XX) max(strwidth(XX, cex=cex)) ) + 
               strwidth("M")*col.margin
