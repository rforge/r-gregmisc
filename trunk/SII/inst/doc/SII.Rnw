    ...
    for(topic in topics)
      {
        ...
        callObj <- call("help", topic=topic, package=package, offline=TRUE)
        
        ## Get the file path
        help.file<- eval(callObj)

        ## Get the latex text
        help.latex <- readLines(help.file)

        ## display it
        cat(paste(help.latex, collapse="\n")) 
...
xlist <- sort(c(sii.constants[, 1], freq))
...
doplot <- function(logx=FALSE, separate=FALSE)
  {
    ...
    tmp <- function() {
      ...
      lines(sii.constants[, 1], sii.constants[, "Ti'.THDN"], col="red", lwd=3)
      abline(v=sii.freqs,lty=3)
    }
    
    tmp()
    lines( x=sii.constants[, 1], y=ispl, col="blue", lwd=2)
    
    if(separate) tmp()
    lines( x=sii.constants[, 1], y=ispl.l, col="cyan", lwd=2)
...
SII:::reload.constants(xls.path="./SII/data")
@ 
...
tab <- cbind(
             matlab=sii.constants[,"Ti'.THDN"],
             R     =sii.left$table[,"T'i"],
             delta =NA
             )
tab[,3] <- tab[,1] - tab[,2]
rownames(tab) <- sii.constants[,".NFreqLin"]
t(round(tab,2))
@ 
...
compare.plot(sii.left, matlab=sii.constants[, "Ti'.THDN"], title="Spline method comparison, Left Ear")
