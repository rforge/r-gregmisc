...
and pharmaceutical research.  An important challenge in designing
and analyzing experiments employing microarrays is the huge number
of gene expression variables collected for each sample, making it
difficult to apply traditional sample size calculation techniques.
We present a simple method for performing and visualizing sample
size calculations for microarray experiments.  We further
demonstrate, via simulation studies, that our method performs well
under a variety of departures from our simplifying assumptions,
except in the case where the underlying sample size calculation
formula employed is inappropriate.  In this latter case, the method
is easily adapted by substituting an appropriate sample size
formula.  The \texttt{ssize} R package implementing the described
method is available from the authors and from the Bioconductor
...
%\section{Contact:} \href{ $^{\rm a}$ $ Gregory.R.Warnes@Pfizer.com $ \\
...
research, such as neuroscience (\fullcite{Mandel03}), pharmacogenomic
research, genetic disease and cancer diagnosis (\fullcite{Heller02}).
...
a small number of replicates (2 to 8) (\fullcite{Speed03}). In many
...
\begin{table}
  \caption{Tested Variables in Simulation Study.
	FDR indicates the Benjamini and Hochberg procedure (1995) to
...
procedure (\fullcite{Benjamini95}) for False Discovery Rate (FDR).
...
(\fullcite{Benjamini95}) and Storey's q-value procedure to control
FDR or positive FDR (pFDR) (\fullcite{Storey02}).  Both the FDR and
...
Benjamini and Hochberg procedure (\fullcite{Benjamini95}). As
