...
obtained from real data.  While the sample size estimation method
does not explicitly account for the interdependencies among genes,
we found that its performance was unaffected by the level of
dependency. We also found that the true proportion of
...
An R package, \texttt{exp.ssize}, implementing the described
...
research, such as neuroscience (\citealp{Mandel03}),
pharmacogenomic research, genetic disease and cancer diagnosis
(\citealp{Heller02}), etc. As a tool for estimating gene
expression and SNP (Single Nucleotide Polymorphism) genotyping,
microarrays produce huge amounts of data which are providing new
insights in these disciplines.
...
including: Should one use one or multiple value for power? Should
...
\item{Summarize the necessary sample size across all genes using a
      ...
      plot is given in Figure \ref{fig:CumNPlot} for which we assume equal sample size for both groups, $n = n_1 = n_2$.}
...
sampling standard deviations (for independent genes) and
...
Irizarry, et. al. (\citealp{Irizarry03}).  A specified fraction
$a$ of the genes is randomly selected as exhibiting a true change
in expression consequent to the applied treatment.  For these
genes, the mean is set to the selected effect size $\Delta$.  For
all other genes, the mean is set to $0$.

Wet then mimic the process of using our method in practice. Thus,
...
The test from each population are then pooled to determine the
...
proportions of genes that are in fact interdependent ($\gamma$) in
...
\begin{figure}[h]
  \centerline{\includegraphics*[width=3.5 in]{ResMtd.pdf}}
                                %\centerline{\includegraphics*[width=3.5in]{f3cv.pdf}}
  %\center{\Huge This plot is not in CVS, so I don't have it. Please
  %          check it into CVS!}
...
Both of these were expected a-priori due to the specific
...
provide a better estimate, but has the drawback of requiring
information difficult to obtain before running the experiment.  We
are currently investigating other methods for applying FDR-control
during the sample size estimation process in the hopes of location
a method that does not suffer from this drawback.
...
\bibitem[Irizarry {\it et~al}., 2003]{Irizarry03} Irizarry, R.A., Hobbs, B., Collin, F., Beazer-Barclay, Y.D., Antonellis, K.J.,
...
\bibitem[Yang and Speed, 2003]{Speed03} Yang, Y. H., Speed, T. {Design and analysis of comparative microarray experiments
...
\bibitem[Zien {\it et~al.}, 2003]{Zien03} Zien, A., Fluck, J., Zimmer, R., Lengauer, T. (2003)
